...
export const Watch = {
	...
	start: () => {
		browsersync.watch([ // watch all content and src files
			...
			Path.normalize(`${ SETTINGS.get().folder.component }/**/*.js`),
...
export const UpdateChange = ( path, history, _doEverything = false ) => {
	...
	const _isReact = path.startsWith( SETTINGS.get().folder.component );
	...
	// double save
		...
		if(
			history.find( item => item.startsWith( SETTINGS.get().folder.component ) ) !== undefined ||
...
export const UpdateReact = ( startTime, path ) => {
	...
	const page = path.replace( SETTINGS.get().folder.component, '' ).replace( '.js', '' );
...
export const UpdateAll = ( startTime ) => {
	...
	const allComponents = Object.keys( require.cache ).filter( ( key ) => key.startsWith( SETTINGS.get().folder.component ) );
