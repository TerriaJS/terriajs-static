...
export const ParseComponent = ( component ) => {
	...
	return new Promise( ( resolve, reject ) => {
		...
		const componentPath = Path.normalize(`${ SETTINGS.get().folder.component }/${ component }`);
...
export const BuildPropsYaml = ( object ) => {
	...
	return new Promise( ( resolve, reject ) => {
		const flags = {
			...
			oneof: ( items ) => `<span class="-flag -flag--default">one of: ` +
				items.map( item => `<span class="-flag__value">${ item.value }</span>` ) +
				`</span>`,
		};

		let props = {};
		let yaml = `<span class="-yaml-line">layout: ${ object.file.slice( 0, -3 ) }</span>\n`;
		let _hasBody = false;
		...
		resolve({
			...
			yaml: <sillywrapper dangerouslySetInnerHTML={ { __html: yaml } } />,
		})
	});
};
...
export const BuildHTML = ( object ) => {
	...
	return new Promise( ( resolve, reject ) => {
		...
		object.props._parseMD = ( markdown ) => <sillywrapper key={`${ object.props._ID }-md`} dangerouslySetInnerHTML={ { __html: ParseMD(
			markdown,
			object.props._self,
			{
		...
		if( !object.disabled ) {
			const componentPath = Path.normalize(`${ SETTINGS.get().folder.component }/${ object.file }`);
		...
		resolve({
			...
			component: <sillywrapper dangerouslySetInnerHTML={ { __html: html } } />,
		})
	});
};
...
export const MakePartials = ( amount ) => {
	const html = '<img src="http://via.placeholder.com/700x100?text=partial" class="-partial" /> '.repeat( amount );

	return <sillywrapper dangerouslySetInnerHTML={ { __html: html } } />
};
...
export const MakeIpsum = ( amount ) => {
	...
	return <sillywrapper dangerouslySetInnerHTML={ { __html: output } } />;
};
